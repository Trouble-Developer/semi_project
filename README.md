...
| 이름 | 브랜치 | 담당 영역 |
|------|--------|-----------|
| 안재훈 | `developer_AJH` | 프로젝트 초기 세팅, 게시판 통합 (자유·봉사후기·공지·고객지원 목록·상세·작성·수정·삭제·비밀글), 메인 페이지, 조회수·좋아요·스크랩·신고, 이미지 업로드·썸네일·스케줄러, 에러 페이지, 헤더 인터셉터 |
| 전재민 | `dev_JJM` | 회원가입 (이메일 인증·주소·유효성 검사), 로그인, 마이페이지 (내 작성글·댓글단 글·스크랩·개인정보 수정·비밀번호 변경), 정보게시판 스크랩 |
| 현동근 | `dev_HDK` | 관리자 페이지 전체 (회원 관리·공지사항 관리·고객지원 관리·신고글 목록·신고 상세·접근 제어) |
| 홍윤기 | `dev_HYG` | 봉사 정보 게시판 전체 (1365 Open API 연동·DB 동기화·지역 필터·CSS) |
| 조창래 | `dev_CCL` | 댓글 시스템 (CRUD·좋아요·신고·대댓글·권한 관리), 아이디 찾기 (이메일 인증 포함), 비밀번호 찾기·재설정, 회원 탈퇴 |
